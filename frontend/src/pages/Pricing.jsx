...
const plans = [
  ...
  },
  {
    ...
    features: ['5,000 analyses/month', 'Full AI insights', 'REST API access', '5 API keys', 'Priority queue'],
...
export default function Pricing() {
  ...
  return (
    <div className="bg-[#1A1410] min-h-screen selection:bg-[#C41E3A]/30">
      <nav className="fixed top-0 left-0 right-0 z-50 border-b border-[#1A1410]/5 bg-[#1A1410]/50 backdrop-blur-xl">
        <div className="max-w-[1720px] mx-auto flex h-20 items-center justify-between px-8 md:px-12">
            ...
            <div className="flex items-center gap-8">
                <Link to="/" className="text-[11px] font-medium tracking-[0.24em] text-[#C41E3A]/60 transition hover:text-[#C41E3A]">NETWORK</Link>
                {token ? (
                    <Link 
                        to="/dashboard" 
                    ...
                    <Link to="/login" className="text-[11px] font-medium tracking-[0.24em] text-[#C41E3A]/60 transition hover:text-[#F5EFE7]">OPERATOR LOGIN</Link>
                )}
            </div>
        </div>
      </nav>
      ...
      <div className="pt-20">
        <ModernPricingPage
            title={
                <>
                    Resource <span className="text-[#C41E3A] text-glow-red">Allocation</span>
                </>
            }
        ...
        <section className="pt-24 border-t border-[#1A1410]/5 grid grid-cols-1 lg:grid-cols-2 gap-24 items-center">
            <div className="space-y-10">
                <div className="space-y-4">
                    ...
                    <h2 className="text-5xl font-display font-black text-[#F5EFE7] uppercase tracking-tight leading-none">Engineered for Technical Mastery</h2>
                ...
                <div className="grid grid-cols-2 gap-8 pt-8">
                    {[
                        { label: 'Uptime SLA', value: '99.99%', icon: Activity, color: 'text-[#C41E3A]' },
                        ...
                        <div key={i} className="space-y-3 p-6 rounded-2xl bg-white/[0.02] border border-[#1A1410]/5 group hover:border-[#1A1410]/10 transition-colors">
                            ...
                            <p className="text-3xl font-display font-black text-[#F5EFE7]">{stat.value}</p>
                            <p className="font-mono text-[9px] text-[#F5EFE7]/30 uppercase tracking-widest">{stat.label}</p>
                        </div>
                    ))}
                </div>
            </div>
            ...
            <div className="glass-card p-12 border border-[#1A1410]/5 relative overflow-hidden group">
                <div className="absolute inset-0 bg-gradient-to-br from-[#C41E3A]/5 via-transparent to-[#C41E3A]/5 opacity-0 group-hover:opacity-100 transition-opacity duration-1000" />
                <div className="relative z-10 space-y-10">
                    <div className="flex items-center justify-between">
                        <span className="font-mono text-[10px] text-[#F5EFE7]/40 uppercase tracking-[0.3em] font-black flex items-center gap-3">
                            <div className="w-1.5 h-1.5 rounded-full bg-[#C41E3A] animate-pulse" />
                            Neural SDK V4.2
                        </span>
                        <ChevronRight className="w-4 h-4 text-[#F5EFE7]/10 group-hover:text-[#C41E3A] transition-colors" />
                    </div>
                    
                    <div className="space-y-6">
                        <div className="p-6 rounded-2xl bg-[#2A1A15]/60 border border-[#1A1410]/10 shadow-inner">
                            <code className="font-mono text-xs text-[#F5EFE7]/60 leading-relaxed block overflow-x-auto whitespace-pre">
                                <span className="text-[#C41E3A]">import</span> Beatzy <span className="text-[#C41E3A]">from</span> <span className="text-[#C41E3A]">'@beatzy/neural-sdk'</span>;{'\n'}
                                <span className="text-[#F5EFE7]/30">{"//"} Initialize cluster node</span>{'\n'}
                                <span className="text-[#C41E3A]">const</span> node = <span className="text-[#FFDAB9]">new</span> Beatzy({'{'}{'\n'}
                                {'  '}apiKey: <span className="text-[#C41E3A]">'BZ_9081X_...'</span>{'\n'}
                                {'}'});
                            </code>
                        </div>
                        <p className="font-mono text-[9px] text-[#F5EFE7]/20 uppercase tracking-[0.2em] text-center italic">Continuous analysis capability active. Distributed nodes online.</p>
                    </div>
                    ...
                    <div className="flex justify-between items-center opacity-30 group-hover:opacity-60 transition-opacity">
                         {['AudioLabs', 'Spectral', 'VocalID', 'Waveform'].map(b => (
                             <span key={b} className="font-display font-black text-[10px] uppercase tracking-widest text-[#F5EFE7]">{b}</span>
                         ))}
                    </div>
                </div>
            </div>
      ...
      <footer className="relative z-10 border-t border-[#1A1410]/5 bg-[#1A1410] px-8 py-20">
        <div className="max-w-[1720px] mx-auto flex flex-col md:flex-row items-center justify-between gap-12">
          <div className="space-y-4 text-center md:text-left">
              ...
              <p className="text-[10px] text-[#C41E3A]/60 uppercase tracking-widest max-w-xs leading-relaxed">Music intelligence engine for identification, spectral analysis, and neural API access.</p>
          </div>
          
          <div className="flex flex-wrap justify-center gap-12">
            {['Architecture', 'Privacy Protocol', 'Technical Docs', 'Service Terms'].map(l => (
              <Link key={l} to="/" className="text-[9px] font-black uppercase tracking-[0.2em] text-[#C41E3A]/60 hover:text-[#C41E3A] transition-colors">{l}</Link>
            ))}
          </div>
