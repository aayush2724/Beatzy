  ...
  return (
    <PageWrapper className="space-y-gutter pb-16">
      ...
      <header className="mb-12 border-b border-white/5 pb-8">
        <h1 className="text-5xl font-headline font-extrabold text-white tracking-tighter" style={SG}>Artist Echoes</h1>
        ...
        <section className="md:col-span-8 flex flex-col gap-8">
          ...
          <div className="glass-panel rounded-xl p-8 relative overflow-hidden h-[460px] flex flex-col border border-glass-border shadow-2xl">
            ...
            <div className="relative z-20 flex justify-between items-start">
              <div>
                <h2 className="font-headline text-xl font-bold text-white tracking-tight">Spectral Resonance</h2>
              ...
              <div className="text-right">
                <p className="font-mono text-[8px] text-white/30 uppercase tracking-widest">Buffer Status</p>
            ...
            <div className="absolute bottom-8 right-8 flex gap-8 z-20">
              <div className="text-right border-r border-white/10 pr-8">
                ...
                <p className="font-mono text-xs font-bold text-white mt-1">192kHz / 32-bit</p>
              </div>
              <div className="text-right">
                <p className="font-mono text-[8px] text-on-surface-variant uppercase tracking-widest">Protocol</p>
                <p className="font-mono text-xs font-bold text-white mt-1">v4.2.1-NEURAL</p>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
                ...
                { label: 'Timbre', val: 'CRISP', sub: 'Spec', color: 'text-white' },
                { label: 'Entropy', val: '0.42', sub: 'Bit', color: 'text-white/60' }
            ].map(s => (
                <div key={s.label} className="glass-panel rounded-xl p-5 border border-glass-border hover:border-white/20 transition-all cursor-default group">
                    <p className="font-mono text-[8px] text-white/30 uppercase tracking-widest mb-2">{s.label}</p>
                    <div className="flex justify-between items-end">
                        <span className={clsx("text-lg font-mono font-bold group-hover:scale-105 transition-transform", s.color)}>{s.val}</span>
                        <span className="text-[8px] font-mono text-white/20 uppercase">{s.sub}</span>
                    </div>
                </div>
            ))}
          </div>
              ...
              {[
                ...
                <div key={item.name} className="flex items-center justify-between group">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="font-mono text-[10px] uppercase tracking-wider text-white/80">{item.name}</span>
                  </div>
