  ...
  return (
    <PageWrapper className="space-y-gutter pb-16">
      ...
      <header className="mb-12 border-b border-line-subtle pb-8">
        <h1 className="text-5xl font-headline font-extrabold text-ink tracking-tighter" style={SG}>Artist Echoes</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-8">
        ...
        <section className="md:col-span-8 flex flex-col gap-8">
          ...
          <div className="glass-panel rounded-xl p-8 relative overflow-hidden h-[460px] flex flex-col border border-glass-border shadow-2xl">
            ...
            <div className="absolute inset-0 bg-[linear-gradient(color-mix(in_oklab,var(--ink)_1%,transparent)_1px,transparent_1px),linear-gradient(90deg,color-mix(in_oklab,var(--ink)_1%,transparent)_1px,transparent_1px)] bg-[size:40px_40px] pointer-events-none"></div>
            
            <div className="relative z-20 flex justify-between items-start">
              <div>
                <h2 className="font-headline text-xl font-bold text-ink tracking-tight">Spectral Resonance</h2>
              ...
              <div className="text-right">
                <p className="font-mono text-[10px] text-ink/30 uppercase tracking-widest">Buffer Status</p>
            ...
            <div className="absolute bottom-8 right-8 flex gap-8 z-20">
              <div className="text-right border-r border-line pr-8">
                <p className="font-mono text-[10px] text-on-surface-variant uppercase tracking-widest">Rate</p>
                <p className="font-mono text-xs font-bold text-ink mt-1">192kHz / 32-bit</p>
              </div>
              <div className="text-right">
                <p className="font-mono text-[10px] text-on-surface-variant uppercase tracking-widest">Protocol</p>
                <p className="font-mono text-xs font-bold text-ink mt-1">v4.2.1-NEURAL</p>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
                ...
                { label: 'Timbre', val: 'CRISP', sub: 'Spec', color: 'text-ink' },
                { label: 'Entropy', val: '0.42', sub: 'Bit', color: 'text-ink/60' }
            ].map(s => (
                <div key={s.label} className="glass-panel rounded-xl p-5 border border-glass-border hover:border-line transition-all cursor-default group">
                    <p className="font-mono text-[10px] text-ink/30 uppercase tracking-widest mb-2">{s.label}</p>
                    <div className="flex justify-between items-end">
                        <span className={clsx("text-lg font-mono font-bold group-hover:scale-105 transition-transform", s.color)}>{s.val}</span>
                        <span className="text-[10px] font-mono text-ink/20 uppercase">{s.sub}</span>
                    </div>
                </div>
            ))}
          </div>
        ...
        <aside className="md:col-span-4 flex flex-col gap-8">
              ...
              {[
                ...
                <div key={item.name} className="flex items-center justify-between group">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="font-mono text-[10px] uppercase tracking-wider text-ink/80">{item.name}</span>
                  </div>
                  <label className="relative inline-flex items-center cursor-pointer">
                    <input type="checkbox" defaultChecked={item.checked} className="sr-only peer" />
                    <div className="w-9 h-5 bg-ink/5 rounded-full peer peer-checked:after:translate-x-full after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-ink after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:bg-primary"></div>
                  </label>
                </div>
              ))}
            </div>
          ...
          <section className="glass-panel rounded-xl border border-glass-border overflow-hidden flex flex-col">
            <div className="p-4 border-b border-glass-border bg-ink/[0.01]">
            ...
            <div className="p-5 space-y-5 overflow-y-auto max-h-[160px] scroll-smooth">
              {[
                ...
                <div key={idx} className="flex gap-4 items-start border-l border-line pl-4 relative">
                  <div className="absolute -left-[3px] top-1 w-1.5 h-1.5 rounded-full bg-primary" />
                  <div>
                    <p className="font-mono text-[10px] text-ink/30 uppercase tracking-widest">{event.time} · {event.status}</p>
                    <p className="font-sans text-xs text-ink/80 mt-1 leading-relaxed">{event.text}</p>
                  </div>
                </div>
              ))}
            </div>
