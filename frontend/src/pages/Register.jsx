    ...
    <AuthShell>
      <div className="w-full max-w-md mx-auto space-y-10 animate-page-entrance">
        ...
        <div className="text-center space-y-3">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full border border-[#f4a460]/20 bg-[#f4a460]/5 text-[#f4a460] font-mono text-[9px] uppercase tracking-[0.2em] mb-4">
            <Cpu className="w-3 h-3" /> New Node Provisioning
          </div>
          <h1 className="text-5xl font-display font-black text-white uppercase tracking-tighter leading-none">Register <span className="text-[#f4a460] text-glow-sandy">Operator</span></h1>
        ...
        <div className="glass-card p-10 border border-white/10 relative overflow-hidden group">
            <div className="absolute inset-0 bg-gradient-to-br from-[#f4a460]/5 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-1000" />
            ...
            <form onSubmit={handleSubmit} className="space-y-6 relative z-10">
                <div className="space-y-2">
                    ...
                    <div className="relative group/input">
                        <User className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-white/10 group-focus-within/input:text-[#f4a460] transition-colors" />
                        <input
                            type="text"
                            required
                            value={formData.name}
                            onChange={(e) => setFormData({ ...formData, name: e.target.value })}
                            className="w-full h-14 bg-white/[0.03] border border-white/10 rounded-2xl pl-12 pr-4 text-white placeholder:text-white/20 focus:outline-none focus:border-[#f4a460]/40 transition-all"
                ...
                <div className="space-y-2">
                    ...
                    <div className="relative group/input">
                        <Mail className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-white/10 group-focus-within/input:text-[#f4a460] transition-colors" />
                        <input
                            type="email"
                            required
                ...
                <div className="space-y-2">
                    ...
                    <div className="relative group/input">
                        <Lock className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-white/10 group-focus-within/input:text-[#f4a460] transition-colors" />
                        <input
                            type="password"
                            required
                            minLength={8}
                ...
                <button
                    type="submit"
                    disabled={loading}
                    className="w-full h-16 rounded-2xl bg-[#f4a460] text-black font-black text-xs uppercase tracking-[0.2em] shadow-[0_0_40px_rgba(244,164,96,0.2)] hover:scale-[1.02] active:scale-[0.98] transition-all flex items-center justify-center gap-3 disabled:opacity-50"
                >
                    {loading ? (
        ...
        <div className="text-center space-y-6">
            <p className="text-[10px] font-mono text-on-surface-variant uppercase tracking-widest leading-relaxed">
                ...
                <Link to="/terms" className="text-white hover:text-[#f4a460] transition-colors">Service Terms</Link> and <Link to="/privacy" className="text-white hover:text-[#f4a460] transition-colors">Privacy Protocol</Link>.
            </p>
            <div className="flex justify-center items-center gap-4">
                <span className="text-[11px] text-white/30 uppercase tracking-widest">Already Registered?</span>
                <Link to="/login" className="group flex items-center gap-2 text-[11px] font-black text-[#f4a460] uppercase tracking-widest hover:text-white transition-colors">
