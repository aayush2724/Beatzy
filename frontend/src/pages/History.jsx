...
export default function History() {
  ...
  return (
    <PageWrapper className="space-y-12 pb-20 animate-page-entrance">
        {/* Header */}
        <header className="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-10 border-b border-[#0D0808]/5 pb-12 relative overflow-hidden">
            ...
            <div className="flex flex-col sm:flex-row items-center gap-4 w-full lg:w-auto relative z-10">
                ...
                <Link to="/upload" className="w-full sm:w-auto flex items-center justify-center gap-3 px-8 py-4 rounded-xl bg-[#FF6B35] text-black font-black text-[10px] uppercase tracking-[0.15em] shadow-[0_0_30px_rgba(255,107,53,0.15)] hover:scale-105 transition-all">
        ...
        {loading ? (
            ...
            <div className="h-80 obsidian-panel rounded-[3rem] border border-dashed border-[#0D0808]/5 flex flex-col items-center justify-center text-center p-12">
                <div className="w-20 h-20 bg-white/5 rounded-[2rem] flex items-center justify-center mb-8 border border-[#0D0808]/5">
            ...
            <>
                ...
                {/* Pagination */}
                {pagination.pages > 1 && (
                    <div className="flex flex-col sm:flex-row justify-center items-center gap-8 pt-16 border-t border-[#0D0808]/5">
                        <button 
                            disabled={page === 1}
                            onClick={() => setPage(p => p - 1)}
                            className="flex items-center gap-3 px-8 py-4 rounded-xl border border-[#0D0808]/5 bg-white/[0.02] text-[#FFFFFF]/40 hover:text-[#FFFFFF] hover:border-[#0D0808]/20 transition-all disabled:opacity-10 font-mono text-[10px] font-black uppercase tracking-[0.2em]"
                        >
                            <ChevronLeft className="w-4 h-4" /> Previous Sector
                        </button>
                        <div className="px-6 py-2 rounded-lg bg-white/5 border border-[#0D0808]/5">
                            <span className="font-mono text-[10px] text-[#FFFFFF] font-black uppercase tracking-[0.2em]"><span className="text-[#FF6B35]">{page}</span> <span className="opacity-20 mx-2">/</span> {pagination.pages}</span>
                        </div>
                        <button 
