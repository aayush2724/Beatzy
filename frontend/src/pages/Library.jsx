      ...
      <header className="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-8 border-b border-[#0D0808]/5 pb-12">
        ...
        <div className="flex flex-col sm:flex-row items-center gap-4 w-full lg:w-auto">
            <div className="relative w-full sm:w-80 group">
                <div className="absolute inset-0 bg-[#FF6B35]/5 blur-[15px] opacity-0 group-focus-within:opacity-100 transition-opacity" />
                <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-[#FFFFFF]/20 w-4 h-4 group-focus-within:text-[#FF6B35] transition-colors" />
                <input 
                    type="text" 
                    value={searchQuery}
                    onChange={(e) => setSearchSearchQuery(e.target.value)}
                    placeholder="Filter registry..."
                    className="w-full h-12 bg-white/[0.03] border border-[#0D0808]/10 rounded-xl pl-12 pr-4 text-[#FFFFFF] text-xs placeholder:text-[#FFFFFF]/20 focus:outline-none focus:border-[#FF6B35]/30 transition-all font-mono uppercase tracking-widest"
                />
            </div>
            <Link to="/compare" className="w-full sm:w-auto flex items-center justify-center gap-3 px-6 py-4 rounded-xl border border-[#0D0808]/10 bg-white/[0.03] text-[#FFFFFF] font-mono text-[10px] uppercase tracking-widest hover:bg-white/[0.06] transition-all">
      ...
      {loading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {Array.from({ length: 6 }).map((_, i) => (
                <div key={i} className="h-72 rounded-[2.5rem] bg-white/[0.02] border border-[#0D0808]/5 animate-pulse" />
            ))}
        </div>
      ) : error ? (
        <div className="glass-card p-12 text-center border border-red-500/20 max-w-xl mx-auto">
          ...
          <button onClick={load} className="px-8 py-3 rounded-xl bg-white text-black font-black text-[10px] uppercase tracking-widest hover:bg-[#FF6B35] transition-all">Retry Link</button>
        </div>
      ) : (
        <div className="space-y-24">
          <section className="space-y-10">
            <div className="flex justify-between items-center">
                <h2 className="font-display font-black text-xl text-[#FFFFFF] uppercase tracking-[0.2em] flex items-center gap-4">
                    <span className="w-8 h-px bg-[#0D0808]/30" /> 
                ...
                {favorites.map((job) => (
                  <div key={job.id} className="relative group">
                    ...
                    <button
                      ...
                      className="absolute top-6 right-6 w-10 h-10 rounded-full bg-[#1A1010]/60 backdrop-blur-md border border-[#0D0808]/10 flex items-center justify-center text-[#FFFFFF] opacity-0 group-hover:opacity-100 transition-all hover:scale-110"
          ...
          <section className="space-y-10">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-8">
                ...
                <form onSubmit={handleCreateCollection} className="flex gap-3 w-full sm:w-auto group">
                  <div className="relative">
                    <FolderPlus className="absolute left-4 top-1/2 -translate-y-1/2 text-[#FFFFFF]/20 w-4 h-4" />
                    <input 
                        className="h-12 bg-white/[0.03] border border-[#0D0808]/10 rounded-xl pl-12 pr-4 text-[#FFFFFF] text-xs placeholder:text-[#FFFFFF]/20 focus:outline-none focus:border-[#FFDAB9]/30 transition-all font-mono uppercase tracking-widest"
                        value={newCollection} 
                        onChange={(e) => setNewCollection(e.target.value)} 
                        placeholder="Label Identifier" 
                    />
                  </div>
                  <button type="submit" className="px-6 h-12 rounded-xl bg-[#FFDAB9] text-[#FFFFFF] font-black text-[10px] uppercase tracking-widest hover:scale-105 transition-all shadow-[0_0_30px_rgba(232,160,132,0.2)]">
                    Initialize
                  </button>
                </form>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              {collections.map((c) => (
                <div key={c.id} className="obsidian-panel p-6 rounded-[2rem] border border-[#0D0808]/5 hover:border-[#FFDAB9]/30 transition-all cursor-pointer group flex flex-col justify-between h-48 relative overflow-hidden">
