      ...
      <div className="flex flex-wrap justify-center gap-6">
        {DEFAULT_ARCHETYPES.map((a) => (
          <div
            ...
            className={`archetype-card cursor-pointer group flex flex-col items-center p-6 glass-card rounded-lg w-[160px] transition-all ${selected === a.id ? 'active border-primary/40' : 'border-glass-border hover:border-primary/20'}`}
          >
            <div className="w-20 h-20 mb-4 rounded-full overflow-hidden border-2 border-primary/20 flex items-center justify-center bg-surface-container-high">
              {a.img ? <img src={a.img} alt={a.name} className="w-full h-full object-cover" /> : <span className="material-symbols-outlined text-3xl text-primary">music_note</span>}
            </div>
