    ...
    <div
      ...
      className={clsx(
        ...
        isDragActive && !isDragReject 
          ...
          : 'border-[#0D0808]/10 hover:bg-white/[0.04] hover:border-white/25 hover:scale-[1.01]',
    ...
    >
      ...
      <div className="relative z-10 flex flex-col items-center gap-6">
        ...
        <div className={clsx(
          ...
          isDragActive && !isDragReject ? 'bg-white/10 border-white scale-110 shadow-[0_0_30px_rgba(255,255,255,0.15)]' : 'bg-white/5 border-[#0D0808]/10 group-hover:scale-110 group-hover:bg-white/10',
          isDragReject && 'bg-red-500/20 border-red-500/50'
        )}>
          {isDragReject ? (
            <AlertCircle size={40} className="text-red-400" />
          ) : isDragActive ? (
            <Music size={40} className="text-[#FFFFFF] animate-pulse" />
          ) : (
            <Upload size={40} className="text-gray-400 group-hover:text-[#FFFFFF] transition-colors" />
          )}
        </div>

        <div>
          <h3 className="font-headline text-3xl md:text-4xl font-semibold text-[#FFFFFF] mb-4 tracking-tight">
            {isDragReject ? 'Unsupported Format' : isDragActive ? 'Drop audio to extract' : 'Load Audio Signature'}
          </h3>
          <p className="text-gray-400 text-lg font-light">
            Drag & drop your file, or <span className="text-[#FFFFFF] underline decoration-white/30 underline-offset-4 hover:decoration-white transition-all">browse files</span>
          </p>
        ...
        <div className="flex justify-center gap-3 mt-4">
          {['MP3', 'WAV', 'FLAC', 'OGG', 'M4A'].map(ext => (
            <span key={ext} className="text-[11px] font-mono text-gray-500 border border-[#0D0808]/10 rounded-md px-3 py-1 bg-[#1A1010]/40">
              {ext}
            </span>
          ))}
        </div>
