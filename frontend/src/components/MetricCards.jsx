      ...
      {METRICS.map((metric) => (
        <section key={metric.id} className="glass-panel rounded-xl p-8 flex flex-col gap-6">
          <div className="flex items-center justify-between">
            ...
            <span className={`font-label-sm border border-line px-2 py-0.5 rounded text-[10px] uppercase ${metric.statusColor}`}>
              {metric.status}
            </span>
          </div>

          {metric.id === 'timbre' && (
            <div className="bg-surface-container-lowest rounded p-6 border border-line-subtle">
          ...
          {metric.id === 'resonance' && (
            <div className="flex items-center gap-4">
              <div className="flex-1 bg-surface-container-lowest border border-line-subtle rounded px-5 py-3 font-label-sm text-primary">
                Minor 7th | Dim 5
              </div>
