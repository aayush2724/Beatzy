...
export default function Layout() {
  ...
  return (
    <div className="flex h-screen overflow-hidden text-on-surface font-body selection:bg-[#D4860A]/30 relative bg-[#1A1410]">
      ...
      <aside 
        ...
        className={clsx(
          "h-full z-40 bg-[#2D1F18]/80 backdrop-blur-2xl border-r border-[#1A1410]/5 flex flex-col pt-8 pb-6 transition-all duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] select-none relative shrink-0",
          isSidebarHovered ? "w-64" : "w-20"
        )}
      >
        {/* Core Header */}
        <div className="px-6 mb-12 flex items-center space-x-4 overflow-hidden shrink-0">
          <div className="w-8 h-8 rounded-lg bg-[#D4860A]/10 flex items-center justify-center border border-[#D4860A]/20 shrink-0 shadow-[0_0_20px_rgba(163,81,57,0.1)]">
            <Zap className="w-4 h-4 text-[#D4860A] fill-[#D4860A]" />
          </div>
          <motion.div 
            animate={{ opacity: isSidebarHovered ? 1 : 0, x: isSidebarHovered ? 0 : -10 }}
            className="whitespace-nowrap"
          >
            <p className="text-[#1A1410] font-bold text-lg leading-none tracking-[0.2em] font-display">BEATZY</p>
            <p className="font-mono text-[8px] text-[#D4860A]/60 tracking-[0.3em] uppercase mt-1">OS V4.2</p>
          </motion.div>
        </div>
          ...
          {navItems.map(({ to, icon: Icon, label }) => (
            <NavLink
              ...
              className={({ isActive }) => clsx(
                ...
                isActive
                  ? 'bg-white/[0.05] text-[#1A1410]'
                  : 'text-on-surface-variant hover:text-[#1A1410] hover:bg-white/[0.02]'
              )}
            >
        ...
        <div className="px-3 mt-auto pt-6 border-t border-[#1A1410]/5">
          <div className={clsx(
            ...
            <div className="w-8 h-8 bg-[#D4860A]/10 border border-[#D4860A]/20 text-[#D4860A] rounded-lg flex items-center justify-center text-xs font-bold shrink-0">
            ...
            {isSidebarHovered && (
              ...
              >
                <p className="text-[11px] font-semibold truncate text-[#1A1410] leading-none mb-1">{user?.name}</p>
                <p className="font-mono text-[8px] text-[#D4860A] uppercase tracking-widest leading-none opacity-70">{user?.plan}</p>
              </motion.div>
            )}
          </div>
          ...
          <button
            ...
            className={clsx(
              "flex items-center h-12 mt-2 rounded-xl text-on-surface-variant hover:text-[#1A1410] hover:bg-white/[0.02] transition-all group",
              isSidebarHovered ? "w-full" : "w-12 mx-auto"
            )}
          >
            <div className="w-14 flex justify-center items-center shrink-0">
