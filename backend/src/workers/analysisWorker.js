...
// ── Socket.IO helper (best‑effort, never blocks the pipeline) ───────────────
function emitToUser(userId, event, payload) {
  try {
    const { getIO } = require('../db/socketio');
    getIO().to(`user:${userId}`).emit(event, payload);
  } catch {
    // Socket.IO may not be initialised when worker runs standalone
  }
}
...
const worker = new Worker('audio-analysis', async (job) => {
  ...
  try {
    ...
    try {
      ...
      if (typeof deleteFromS3 === 'function') await deleteFromS3(s3Key);
    } catch { /* best effort */ }

    throw err;
  }

  // ── Persist results (incl. spotify + mood_confidence) ───────────────────
  await pool.query(
